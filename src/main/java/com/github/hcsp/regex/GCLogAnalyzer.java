...
public class GCLogAnalyzer {
    ...
    public static List<GCActivity> parse(File gcLog) throws IOException {
        String youngGenPattern = "\\[PSYoungGen: (\\d+)K->(\\d+)K\\((\\d+)K\\)] ";
        String otherGenPattern = "(?:\\[\\w+: \\d+K->\\d+K\\(\\d+K\\)],? )?";
        String secsPattern = "(?:\\d+\\.?\\d* secs] )?";
        String heapPattern = "(\\d+)K->(\\d+)K\\((\\d+)K\\), ";
        String timesPattern = "\\[Times: user=(\\d+\\.?\\d*) sys=(\\d+\\.?\\d*), real=(\\d+\\.?\\d*) secs]";
        Pattern pattern = Pattern.compile(youngGenPattern + otherGenPattern + heapPattern + otherGenPattern + secsPattern + timesPattern);
