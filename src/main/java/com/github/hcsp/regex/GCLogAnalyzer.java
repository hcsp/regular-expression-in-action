...
public class GCLogAnalyzer {
    ...
    private static final String GCLOG_REGEX = "(\\d+)K->(\\d+)K\\((\\d+)K\\).*?(\\d+)K->(\\d+)K\\((\\d+)K\\)"
            + ".*?user=(\\d\\.\\d{2}).*?sys=(\\d\\.\\d{2}).*?real=(\\d\\.\\d{2})";
    private static final Pattern GCLOG_PATTERN = Pattern.compile(GCLOG_REGEX);

    public static List<GCActivity> parse(File gcLog) throws IOException {
        List<String> lines = Files.readAllLines(gcLog.toPath());
        return lines.stream()
                .map(GCLOG_PATTERN::matcher)
                .filter(Matcher::find)
