...
public class GCLogAnalyzer {
    ...
    private static final Pattern PRE_COMPILE_PATTERN = Pattern.compile("(\\d+)\\w->(\\d+)\\w\\((\\d+).*\\s(\\d+)\\w->(\\d+)\\w\\((\\d+).*user=(\\d+\\.\\d+).*sys=(\\d+\\.\\d+).*real=(\\d+\\.\\d+)");
    public static List<GCActivity> parse(File gcLog) {
        List<GCActivity> GCResult = new LinkedList<>();
        try {
            List<String> lines = Files.readAllLines(gcLog.toPath());
            for (String line : lines) {
                Matcher matcher = PRE_COMPILE_PATTERN.matcher(line);
                while (matcher.find()) {
                    GCResult.add(new GCActivity(
