...
public class GCLogAnalyzer {
    ...
    public static List<GCActivity> parse(File gcLog) {
        Pattern pattern = Pattern.compile("\\[PSYoungGen: ?(\\d+)K->(\\d+)K\\((\\d+)K\\)]" +
                "(?: \\[ParOldGen:)? ?(\\d+)K->(\\d+)K\\((\\d+)K\\)" +
                ".+user=(\\d+\\.\\d+),? ?sys=(\\d+\\.\\d+),? ?real=(\\d+\\.\\d+)");
        List<GCActivity> gcActivityList = new ArrayList<>();
        try {
            List<String> logs = Files.readAllLines(gcLog.toPath());
            for (String string : logs) {
                Matcher matcher = pattern.matcher(string);
                if (matcher.find()) {
                    GCActivity gcActivity = new GCActivity(Integer.parseInt(matcher.group(1)),
