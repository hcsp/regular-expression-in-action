...
public class GCLogAnalyzer {
    ...
    public static List<GCActivity> parse(File gcLog) throws IOException {
        ...
        Pattern gcPattern = Pattern.compile("(\\d+)K->(\\d+)K.(\\d+)K.+\\s(\\d+)K->(\\d+)K\\((\\d+)K\\).+user=(\\d.\\d+).+sys=(\\d.\\d+).+real=(\\d.\\d+)");
        List<GCActivity> GCList = new ArrayList<GCActivity>();
...
//                arrayList.add("a");
                ...
                GCList.add(new GCActivity(youngGenBefore, youngGenAfter, youngGenTotal, heapBefore,
                        heapAfter, heapTotal, user, sys, real));
            }
        }
        System.out.println(arrayList.size());
//        return GCList;

//        Pattern gcPattern = Pattern.compile("PSYoungGen: (\\d+)K->(\\d+)K\\((\\d+)K\\)] (\\d+)K->(\\d+)K\\((\\d+)K\\), \\d.\\d+ secs\\] \\[Times: user=(\\d.\\d+) sys=(\\d.\\d+), real=(\\d.\\d+) secs]");
//        List<GCActivity> GCList = new ArrayList<GCActivity>();
//        for (String line : lines) {
//            System.out.println(line);
//            Matcher matcher = gcPattern.matcher(line);
...
//                int heapTotal = Integer.parseInt(matcher.group(6));
//                double user = Double.parseDouble(matcher.group(7));
//                double sys = Double.parseDouble(matcher.group(7));
//                double real = Double.parseDouble(matcher.group(7));
//
//                GCList.add(new GCActivity(youngGenBefore, youngGenAfter, youngGenTotal, heapBefore,
//                        heapAfter, heapTotal, user, sys, real));
//            }
