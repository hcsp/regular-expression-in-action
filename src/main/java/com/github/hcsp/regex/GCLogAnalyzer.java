...
public class GCLogAnalyzer {
    private static final String GC_TAMP_REGEX = "(\\d+)K->(\\d+)K\\((\\d+)K\\).+?(\\d+)K->(\\d+)K\\((\\d+)K\\).+" +
            "user=(\\d*.\\d{2}).*sys=(\\d*.\\d{2}).*real=(\\d*.\\d{2})";
    private static final Pattern GC_LOG_PATTERN = Pattern.compile(GC_TAMP_REGEX);
    ...
    public static List<GCActivity> parse(File gcLog) throws IOException {
        ...
        return list.stream()
                ...
                .filter(Matcher::find)
                .map(GCLogAnalyzer::creatGCActivity)
