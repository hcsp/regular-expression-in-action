...
public class GCLogAnalyzer {
    ...
    public static List<GCActivity> parse(File gcLog) throws IOException {
        ...
        Pattern gcLogPattern = Pattern.compile("(\\d+)K->(\\d+)K.(\\d+)K.+\\s(\\d+)K->(\\d+)K\\((\\d+)K\\).+user=(\\d.\\d+).+sys=(\\d.\\d+).+real=(\\d.\\d+)");
        for (String line : lines) {
            Matcher matcher = gcLogPattern.matcher(line);
            while (matcher.find()) {
                results.add(new GCActivity(Integer.parseInt(matcher.group(1)),
