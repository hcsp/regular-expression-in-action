...
public class GCLogAnalyzer {
    ...
    //[ParOldGen:
    private static Pattern pattern = Pattern.compile(".*\\[PSYoungGen: (\\d+)K->(\\d+)K\\((\\d+)K\\)] (\\[ParOldGen: \\d+K->\\d+K\\(\\d+K\\)] )?(\\d+)K->(\\d+)K\\((\\d+)K\\),.*user=(\\d\\.\\d+) sys=(\\d\\.\\d+), real=(\\d\\.\\d+).*");
    ...
    public static List<GCActivity> parse(File gcLog) throws IOException {
        ...
        for (String line : lines
            ...
            GCActivity singleLine;
            if ((singleLine = handle(line)) != null) {
    ...
    private static GCActivity handle(String line) {
