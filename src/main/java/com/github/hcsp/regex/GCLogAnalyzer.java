...
public class GCLogAnalyzer {
    ...
    private static final Pattern GC_LOG_PATTERN = Pattern.compile("PSYoungGen: (\\d+)K->(\\d+)K\\((\\d+)K\\)\\]\\s(?:\\[ParOldGen: \\d+K->\\d+K\\(\\d+K\\)\\]\\s)?(\\d+)K->(\\d+)K\\((\\d+)K\\),\\s(?:\\[Metaspace: \\d+K->\\d+K\\(\\d+K\\)\\],\\s)?\\d+\\.\\d+ secs\\] \\[Times: user=(\\d+\\.\\d+) sys=(\\d+\\.\\d+), real=(\\d+\\.\\d+) secs");
        ...
        try {
            ...
            for (String line : lines) {
                ...
                while (matcher.find()) {
                    ...
                    GCActivity gcActivity = new GCActivity(youngGenBefore, youngGenAfter, youngGenTotal,
