...
public class GCLogAnalyzer {
    ...
    private static final Pattern GC_PATTERN = Pattern.compile(GC_LOG_REGEX);


    public static List<GCActivity> parse(File gcLog) throws IOException {
        List<String> strings = Files.readAllLines(gcLog.toPath());
        return strings.stream().map(GCLogAnalyzer.GC_PATTERN::matcher)
                .filter(Matcher::find)
                .map(GCLogAnalyzer::generateGCActivity)
                .collect(Collectors.toList());
    }

    public static GCActivity generateGCActivity(Matcher matcher) {
