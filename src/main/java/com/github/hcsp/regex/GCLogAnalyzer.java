...
public class GCLogAnalyzer {
    ...
    public static List<GCActivity> parse(File gcLog) throws IOException {
        final List<String> list = Files.readAllLines(gcLog.toPath());
        return list.stream().filter(GCLogAnalyzer::isGcInfo)
                .map(GCLogAnalyzer::compositeGCInfo)
                .collect(Collectors.toList());
    }

    private static boolean isGcInfo(String line) {
        Pattern pattern = Pattern.compile("\\[PSYoungGen: (\\d+)K->(\\d+)K\\((\\d+)K\\).*\\]\\D*(\\d+)K->(\\d+)K\\((\\d+)K\\),.+user=((?:\\d\\.?)\\d+).+sys=((?:\\d\\.?)\\d+).+real=((?:\\d\\.?)\\d+)");
        return pattern.matcher(line).find();
    }

    private static GCActivity compositeGCInfo(String line) {
        Pattern pattern = Pattern.compile("\\[PSYoungGen: (\\d+)K->(\\d+)K\\((\\d+)K\\).*\\]\\D*(\\d+)K->(\\d+)K\\((\\d+)K\\),.+user=((?:\\d\\.?)\\d+).+sys=((?:\\d\\.?)\\d+).+real=((?:\\d\\.?)\\d+)");
