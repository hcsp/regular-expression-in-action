...
public class GCLogAnalyzer {
    private static final String GC_LOG_Regex = "(\\d+)K->(\\d+)K\\((\\d+)K\\).+?(\\d+)K->(\\d+)K\\((\\d+)K\\).+" +
            "user=(\\d\\.\\d{2})\\s+sys=(\\d\\.\\d{2}),\\s+real=(\\d\\.\\d{2})";

    private static final Pattern GC_LOG_Pattern = Pattern.compile(GC_LOG_Regex);
    ...
    public static List<GCActivity> parse(File gcLog) throws IOException {
        ...
        return lines.stream().map(GC_LOG_Pattern::matcher).filter(Matcher::find)
                .map(GCLogAnalyzer::buildGCActivity).collect(Collectors.toList());
    }

    public static GCActivity buildGCActivity(Matcher matcher) {
